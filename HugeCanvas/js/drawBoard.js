(function () {
  ...
  */
  ...
  function checkObjVisible(pts, width, height) {
    ...
    var flag = false;
    pts.map(function (pt) {
      flag = checkPointVisible(pt, width, height);
    })
    return flag;
  }
  DrawBoard.prototype.calcObjArgs = function (obj) {
    // 根据viewer测算新的参数值
    var args = [];
    args[0] = obj.args[0] - this.viewer.pos[0];
    args[1] = obj.args[1] - this.viewer.pos[1];
    args[2] = obj.args[2] * this.viewer.scale;
    args[3] = obj.args[3] * this.viewer.scale;
    if (checkObjVisible([
      [args[0], args[1]],
